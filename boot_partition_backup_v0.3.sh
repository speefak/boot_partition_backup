#!/bin/bash
# name          : boot_partition_backup
# desciption    : creates a backup image from boot drive when changes detected.
# licence       : (CC) BY-NC-SA
# version 	: 0.3
# notice 	: if /boot differents from / drive, like booting from PCI NVME disk when BIOS doesn`t support NVME boot
# infosource	:
#
#------------------------------------------------------------------------------------------------------------
############################################################################################################
...
#------------------------------------------------------------------------------------------------------------
 ...
 BootImageStorageDir="/home/"
 BootImageName="$(hostname)-boot-"
 ProcessingDate=$(date '+%F-%H%M%S')
 Logfile="/tmp/bpb-$ProcessingDate.log"
 MailAddress=root
 MailSubjectLine="Boot partition backup $(hostname) ( $(date '+%F %H:%M') )"
...
usage() {
	...
	printf " -mr		=> (m)ail (r)eport to $MailAddress \n"
...
check_boot_device () {
	...
	#  check for /boot changes / Skip md5sum check if no Bootimage exists
	if [[ -n $BootImageList ]]; then
		...
		if [[ $? == 0 ]]; then printf "$CheckMark \n" ; else printf "$Red x $Reset"; fi

		# compare checksums skip update_boot_image if checksum match / does not change
...
update_boot_image () {
	...
	dd if="$BootDevice" of="$BootImageStorageDir$BootImageName$ProcessingDate.img" >/dev/null 2>&1
	if [[ $? == 0 ]]; then printf "$CheckMark \n" ; else printf "$Red x $Reset"; fi
	printf "\n"
}
#------------------------------------------------------------------------------------------------------------------------------------------------
clear_boot_image_storage () {
	printf " delete obsolte images \n"
	BootImageList=$(find "$BootImageStorageDir" -maxdepth 1 -type f -name "$BootImageName*" | sort -u)
...
#------------------------------------------------------------------------------------------------------------------------------------------------
	...
	if [[ -z $Monochrome ]] &&  [[ -z $MailLogReport ]]; then
		load_color_codes
	fi
...
#------------------------------------------------------------------------------------------------------------------------------------------------
	...
	# mail logfile
	if [[ -n $MailLogReport ]] || [[ -n $(cat $Logfile | grep "create backup image" ) ]]; then
		printf " send $Logfile to $MailAddress "
		cat $Logfile | sed 's/\x1B\[[0-9;]*m//g' | mail -a "Content-Type: text/plain" -s "$MailSubjectLine" $MailAddress
