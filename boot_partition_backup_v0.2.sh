...
# desciption    : creates a backup image from boot device when changes detected.
# licence       : (CC) BY-NC-SA
# version 	: 0.2
...
usage() {
	...
	printf " -mr		=> mail report ( mail -a root )\n"
...
#------------------------------------------------------------------------------------------------------------------------------------------------
	...
	if [[ -n $MailLogReport ]]; then
		...
		cat $Logfile | mail -a "Content-Type: text/plain" -s "$MailSubjectLine" $MailAddress
	fi
